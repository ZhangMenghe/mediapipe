...
//   else load_type = FROM_CAMERA;
	if(FLAGS_input_type == "camera") load_type = FROM_CAMERA;
	else if(FLAGS_input_type == "video") load_type = FROM_VIDEO;
	else if(FLAGS_input_type == "frames") load_type = FROM_FRAMES;
	else {load_type = DEBUG_EMPTY; return OkStatus();}
...
cv::Size GPUTask::getFrameSize(){
  if(frame_size.width == 0){
	  if(load_type == FROM_FRAMES || load_type== DEBUG_EMPTY){
		cv::Mat tframe;
...
bool GPUTask::getFrame(cv::Mat& camera_frame){
	if(load_type == DEBUG_EMPTY){

		camera_frame = cv::Mat(768,430, CV_8UC3, cv::Scalar(0,0,255)); //cv::Mat::ones(768, 430, CV_8UC3);
    // LOG(INFO) << camera_frame.size();

	}else if(load_type == FROM_FRAMES){
